...
public class HiveHbaseTest {
    ...
    public static void main(String[] args) throws SQLException {
     ...
     /*   sb = new StringBuilder(" CREATE TABLE hive_tas_app_age_gprs_20140503(STATIS_MONTH STRING,BUSI_ID STRING,KEY_WORD STRING,SECTION_ID STRING,BRAND_ID STRING,WEB_ADDRESS STRING)");
        ...
        sb.append(" TBLPROPERTIES (\"hbase.table.name\" =\"hbase_tas_app_age_gprs_20140503\")");
        ...
        //String sql="select key,value,a from test_hbase4";
       ...
       /* sb=new StringBuilder("ALTER TABLE hive_tas_app_age_gprs_20140503 ADD PARTITION(MONTHID='2014-08-03')");
       ...
       //sb = new StringBuilder("INSERT OVERWRITE TABLE hive_tas_app_age_gprs_20140503 PARTITION (MONTHID='2014-08-03') SELECT STATIS_MONTH,BUSI_ID,KEY_WORD,SECTION_ID,BRAND_ID,WEB_ADDRESS FROM tas_app_age_gprs_20140905");
      /*  sb = new StringBuilder("INSERT OVERWRITE TABLE hive_tas_app_age_gprs_20140503 PARTITION (MONTHID='2014-08-03') SELECT STATIS_MONTH,BUSI_ID,KEY_WORD,SECTION_ID,BRAND_ID,WEB_ADDRESS FROM tas_app_age_gprs_20140905");        
        //sb=new StringBuilder("load data inpath '/tmp/testhanseq.txt' into table hive_tas_app_age_gprs_20140503 partition(MONTHID='2014-08-04')");　异常：A non-native table cannot be used as target for LOAD
        System.out.println("执行导入数据开始：sql="+sb.toString());
        stmt.execute(sb.toString());
        System.out.println("执行导入数据完成");
        
        //查询hive中的数据　select * 不会通过MP查询,
        String sql = "select STATIS_MONTH,BUSI_ID,KEY_WORD,SECTION_ID,BRAND_ID,WEB_ADDRESS from hive_tas_app_age_gprs_20140503" ;
