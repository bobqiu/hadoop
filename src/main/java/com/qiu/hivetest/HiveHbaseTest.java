...
public class HiveHbaseTest {
    ...
    public static void main(String[] args) throws SQLException {
        ...
        sb = new StringBuilder(" CREATE TABLE hive_tas_app_age_gprs_20140803(STATIS_MONTH STRING,BUSI_ID STRING,KEY_WORD STRING,SECTION_ID STRING,BRAND_ID STRING,WEB_ADDRESS STRING)              \n");
        sb.append(" PARTITIONED BY (MONTHID STRING)");
        sb.append(" STORED BY 'org.apache.hadoop.hive.hbase.HBaseStorageHandler'  \n");
        sb.append(" STORED AS");
        sb.append(" INPUTFORMAT 'org.apache.hadoop.mapred.TextInputFormat'");
        sb.append(" OUTPUTFORMAT 'org.apache.hadoop.hive.hbase.HiveHFileOutputFormat'");
        sb.append(" WITH SERDEPROPERTIES (\"hbase.columns.mapping\"=\":key,cf1:BUSIID,cf1:KEYWORD,cf1:SECTIONID,cf1:BRANDID,cf1:WEBADDRESS\")   \n");
        sb.append(" TBLPROPERTIES (\"hbase.table.name\" =\"hbase_tas_app_age_gprs_20140803\")                     \n");
       ...
       /* sb=new StringBuilder("ALTER TABLE hive_tas_app_age_gprs_20140803 ADD PARTITION(MONTHID='2014-08-03')");
        ...
        sb = new StringBuilder("insert overwrite table hive_tas_app_age_gprs_20140803 partition (MONTHID='2014-08-03') select STATIS_MONTH,BUSI_ID,KEY_WORD,SECTION_ID,BRAND_ID,WEB_ADDRESS from tas_app_age_gprs_20140905");
        System.out.println("执行导入数据开始：sql="+sb.toString());
        stmt.execute(sb.toString());
        System.out.println("执行导入数据完成");
        
        //查询hive中的数据　select * 不会通过MP查询,
        String sql = "select STATIS_MONTH,BUSI_ID,KEY_WORD,SECTION_ID,BRAND_ID,WEB_ADDRESS from hive_tas_app_age_gprs_20140803" ;
